...
node_1.default.Cloud.define(`autocompleteAdminHome`, async (request) => {
    const { Name } = request.params;
    try {
        const query = new node_1.default.Query('User');
        query.equalTo(`adminUser`, true);
        query.limit(1000000);
        const results = await query.find({ useMasterKey: true });
        if (results) {
            ...
            const response = await Promise.all(objJson.map(async (element) => {
                if (Name.length > 0 && element.username.search(Name) === 0) {
                    return element;
                }
                return null;
            }));
...
node_1.default.Cloud.define(`setAutocompleteAdminHome`, async (request) => {
    const { Name } = request.params;
    try {
        const query = new node_1.default.Query('_User');
        query.equalTo(`fullname`, Name);
        query.equalTo(`adminUser`, true);
        const results = await query.find({ useMasterKey: true });
        if (results) {
            const response = await Promise.all(results.map(async (value) => {
